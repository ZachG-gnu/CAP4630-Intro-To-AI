...
from checkers import Checker, black_squares
import pytest
import numpy as np
...
def test_possible_moves_of_black_player(checker_game, black_squares):
    ...
    checker_game.players[checker_game.current_player -
                         1].pos = [[0, 1], (1, 0), (1, 2)]
    ...
    for pos in possible_moves:  # all possible combination of pieces after black moves.
        ...
        assert len(index_in_pos[0]) == len(
            checker_game.players[checker_game.current_player-1].pos)
...
def test_is_player_position_updated(checker_game):
    ...
    assert checker_game.players[checker_game.current_player -
                                1].pos == index_in_pos
